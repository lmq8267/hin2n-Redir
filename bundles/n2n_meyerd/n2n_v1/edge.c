...
static void supernode2addr(n2n_edge_t * eee, char* addr) {
  ...
  if (strncasecmp(tmpAddr, "txt:", 4) == 0) {
        // 处理txt查询
        const char *domain = tmpAddr + (strncasecmp(tmpAddr, "txt://", 6) == 0 ? 6 : 4);
        ...
        if (ctx.success) {
            snprintf(tmpAddr, sizeof(tmpAddr), "%s", ctx.txt_record);
            // traceEvent(TRACE_NORMAL, "TXT record query successful, the address is: %s", tmpAddr);
        } else {
    ...
    if (strncasecmp(tmpAddr, "http:", 5) == 0 || strncasecmp(tmpAddr, "https:", 6) == 0) {
        char result[8192] = {0};
        CURL *curl;
        CURLcode res;
        char redirect_url[512] = {0};
	// 如果 addr 以 http: 或 https: 开头但没有 //
	if (strncasecmp(tmpAddr, "http:", 5) == 0 && strncasecmp(tmpAddr, "http://", 7) != 0) {
    		char fixed[512] = {0};
    		snprintf(fixed, sizeof(fixed), "http://%s", tmpAddr + 5); // 添加 //
    		safe_strncpy(tmpAddr, fixed, sizeof(tmpAddr));
	} else if (strncasecmp(tmpAddr, "https:", 6) == 0 && strncasecmp(tmpAddr, "https://", 8) != 0) {
    		char fixed[512] = {0};
    		snprintf(fixed, sizeof(fixed), "https://%s", tmpAddr + 6); // 添加 //
    		safe_strncpy(tmpAddr, fixed, sizeof(tmpAddr));
	}
        // 初始化 libcurl
    ...
    curl_easy_setopt(curl, CURLOPT_URL, tmpAddr); // 设置 URL
    ...
    if (res != CURLE_OK) {
        ...
        // 设置 curl 请求参数（IPv6）
        curl_easy_setopt(curl, CURLOPT_URL, tmpAddr);
        ...
        if (status_code >= 300 && status_code < 400) {
            ...
            safe_strncpy(tmpAddr, redirect_url, sizeof(tmpAddr));
            strip_http_prefix(tmpAddr); // 去掉http://或https://前缀
            //traceEvent(TRACE_NORMAL, "Redirect address detected: %s", tmpAddr);
	    ...
	    if (!body) {
            ...
            if (body) {
                ...
                // 更新 addr 变量
                safe_strncpy(tmpAddr, clean_addr, sizeof(tmpAddr));

                // 去掉 http:// 或 https:// 前缀
                strip_http_prefix(tmpAddr);
                // 打印成功日志
                //traceEvent(TRACE_NORMAL, "HTTP 200 webpage body address: %s", tmpAddr);
            } else {
